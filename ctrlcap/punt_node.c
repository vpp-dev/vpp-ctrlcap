...
static u8 * format_ctrlcap_punt_trace (u8 * s, va_list * args)
{
  ...
  s = format (s, "ctrlcap punt: sw_if_index %d, next index %d",
		  ...
		  vlib_frame_t * frame)
{
  ...
  while (n_left_from > 0)
	...
	{
          u32 next0 = ctrlcap_NEXT_INTERFACE_OUTPUT;
          u32 next1 = ctrlcap_NEXT_INTERFACE_OUTPUT;
...
VLIB_REGISTER_NODE (ctrlcap_punt_node) = {
  ...
  .next_nodes = {
        [ctrlcap_NEXT_INTERFACE_OUTPUT] = "interface-output",
  },
};
