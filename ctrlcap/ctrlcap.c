...
/* Enable/disable and config function shared between message handler and debug CLI */
                            ...
                            int enable_disable)
{
    ...
    // TODO: reapply all redirects (if they were removed when this was disabled)
    ...
    // remember inject interface
    cm->in_sw_if_index = in_sw_if_index;
    // TODO: setup redirect to inject node

    // prepare header
    rv = ip4_nsh_header_prep(&cm->ip4_nsh_hdr,
            src, src_port, dst, dst_port);

    u32 node_index = enable_disable ? ctrlcap_inject_node.index : ~0;
     ...
     */
    rv = vnet_hw_interface_rx_redirect_to_node (cm->vnet_main, in_sw_if_index,
                                                node_index);
    cm->is_enabled = 1;
    return rv;
}

/* set control packet capture for interface */
                 ...
                 u32 sw_if_gid, int enable_disable)
{
    ...
    // TODO: make sure we don't assign same gid to multiple ifaces
                                   ...
                                   vlib_cli_command_t * cmd)
{
    ...
    if (!got_iface)
        return clib_error_return (0, "Please specify interface for incoming nsh encapsulated data.");
    if (src_set == 0)
    ...
    rv = ctrlcap_enable_disable (cm, &src, src_port, &dst, dst_port, sw_if_index, enable_disable);
...
VLIB_CLI_COMMAND (ctrlcap_enable_disable_command, static) = {
    .path = "ctrlcap config",
    .short_help = 
    "ctrlcap config src <source-ipv4>:<source port> dst <destination-ipv4>:<destination port> interface <interface-name> [disable]",
...
(vl_api_ctrlcap_enable_disable_t * mp)
{
    ...
    ctrlcap_main_t * cm = &ctrlcap_main;
    u32 sw_if_index = ntohl (mp->in_sw_if_index);
    int rv;
    ip4_address_t src, dst;
    ...
    rv = ctrlcap_enable_disable (cm,
            ...
            sw_if_index, (int) (mp->enable_disable));
